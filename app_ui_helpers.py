...
def build_book_options(books):
    ...
    options = {}
    for book in books:
        book_id, book_name, volume_name, _description = book
        label = f"{book_name}" if not volume_name else f"{book_name}（{volume_name}）"
        options[label] = book_id
    return options


def build_unit_options(units):
    """
    把 units 查询结果转成：
    {unit_name: unit_id}
    """
    return {unit[1]: unit[0] for unit in units}
...
def parse_part2_to_two_column_rows(part2_text: str):
    """
    把 Part 2 文本解析成双栏练习表格。
    ...
    half = (len(item_rows) + 1) // 2
    left_items = item_rows[:half]
    right_items = item_rows[half:]
    ...
    for i in range(max_len):
        ...
        if i < len(left_items):
            left_no, left_text = left_items[i]
        if i < len(right_items):
            right_no, right_text = right_items[i]

        table_rows.append(
            {
                "No": left_no,
                "Meaning / Word": left_text,
                "Answer": "",
                "No_2": right_no,
                "Meaning / Word_2": right_text,
                "Answer_2": "",
            }
        )
...
def build_part2_html_table(rows):
    ...
    for row in rows:
        ...
        right_no = escape(str(row.get('No_2', '')))
        right_text = escape(str(row.get('Meaning / Word_2', '')))
        right_answer = escape(str(row.get('Answer_2', '')))
        html_parts.append(
            f"""
        <tr>
...
def render_html_block(html_body: str, height: int = 500):
    full_html = f"""
    <html>
    <head>
        <meta charset="utf-8">
...
def render_lesson_parts(parts: dict):
    ordered_keys = ["part1", "part1_review", "part2", "part3", "part4", "part5", "part6", "part7"]
    title_map = {
        "part1": "Part 1 Key Vocabulary",
        "part1_review": "Part 1B Personal Review Words",
        "part2": "Part 2 Vocabulary Consolidation Practice",
    ...
    for key in ordered_keys:
        ...
        with st.expander(title, expanded=(key in ['part1', 'part1_review', 'part2'])):
            if key in ['part1', 'part1_review']:
                rows = parse_part1_table(parts[key])
                if rows:
                    render_html_block(build_part1_html_table(rows), height=420)
                else:
                    st.text_area(label=f'{title}_preview', value=parts[key], height=250, key=f'preview_{key}')
            elif key == 'part2':
                rows = parse_part2_to_two_column_rows(parts[key])
                if rows:
                    render_html_block(build_part2_html_table(rows), height=700)
                else:
                    st.text_area(label=f'{title}_preview', value=parts[key], height=250, key=f'preview_{key}')
            elif key == 'part3':
                render_html_block(build_part3_html(parts[key], highlight_words), height=760)
            elif key == 'part4':
                render_html_block(build_part4_html(parts[key]), height=900)
            elif key == 'part5':
                render_html_block(build_part5_html(parts[key]), height=760)
            elif key == 'part6':
                render_html_block(build_part6_html(parts[key]), height=1200)
            elif key == 'part7':
                render_html_block(build_part7_html(parts[key]), height=1000)
            else:
                st.text_area(label=f'{title}_preview', value=parts[key], height=250, key=f'preview_{key}')
...
def build_full_lesson_preview_html(parts: dict):
    ...
    if 'part2' in parts and parts['part2']:
        rows = parse_part2_to_two_column_rows(parts['part2'])
        if rows:
...
# ==============================
@st.dialog("学生详情")
def show_student_detail_dialog(student_id):
    """
    弹出学生详情窗口。
    """
    summary = db.get_student_detail_summary(student_id)
    lesson_details = db.get_student_lessons_with_new_words(student_id)

    if not summary:
        st.warning("没有找到该学生的信息。")
        return

    student_id, name, grade, lesson_count, vocab_count = summary

    st.subheader(f"{name}（{grade}）")
    st.write(f"总学案数：{lesson_count}")
    st.write(f"总词汇数：{vocab_count}")
    st.markdown("---")
    ...
    if not lesson_details:
        ...
        for lesson in lesson_details:
            st.markdown(
                f"### 学案 ID：{lesson['lesson_id']} | {lesson['lesson_type']} | 难度：{lesson['difficulty']}"
            )
            st.write(f"主题：{lesson['topic']}")
            st.write(f"创建时间：{lesson['created_at']}")

            if lesson["new_words"]:
                with st.expander("查看本学案新词表", expanded=False):
                    for i, (word, meaning) in enumerate(lesson["new_words"], start=1):
    ...
    if not book_progress:
        ...
        for row in book_progress:
            ...
            label = f"{book_name}" if not volume_name else f"{book_name}（{volume_name}）"
            progress_ratio = (learned_count / total_count) if total_count > 0 else 0
    ...
    test_records = db.get_student_vocab_test_records(student_id, limit=20)
    if not test_records:
        st.info("这个学生目前还没有词汇检测记录。")
    else:
        for row in test_records:
            ...
            item_rows = db.get_vocab_test_record_items(test_record_id)
            if item_rows:
                with st.expander("查看本次检测明细", expanded=False):
                    for idx, item in enumerate(item_rows, start=1):
                        ...
                        st.write(
                            f"{idx}. [{mode}] {word} - {meaning} | "
                            f"你的答案：{user_answer if user_answer else '（未作答）'} | "
                            f"结果：{result_text}"
                        )
